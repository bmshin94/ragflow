...
## 프로젝트 개요
복잡한 문서와 기업 데이터를 깊이 있게 읽고 AI 에이전트와 완벽히 결합하여 답변의 신뢰도를 극대화하는 "차세대 RAG 지식 검색 엔진"
PDF, 표, 복잡한 서식 속 텍스트까지 사람처럼 정교하게 파악하여 에이전트가 완벽한 사실 기반으로 일하도록 지원
사내 문서가 아무리 방대해도 단 한 번의 질문으로 정확한 출처와 답을 짚어내어 AI의 거짓말(환각)을 제거
